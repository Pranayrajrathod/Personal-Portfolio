export const Bio = {
  name: "Pranay Raj Rathod Pathlavath",
  roles: [
    "Full Stack Developer",
    "Programmer",
    "AI&ML Enthusiast",
    "NSS Volunteer"
  ],
  description:
    "I am a motivated and versatile individual, always eager to take on new challenges. With a passion for learning I am dedicated to delivering high-quality results. With a positive attitude and a growth mindset, I am ready to make a meaningful contribution and achieve great things.",
  github: "https://github.com/Pranayrajrathod",
  resume:
    "https://drive.google.com/file/d/1YkW0kGJuFJf1Z6q7DPgksFI6gGdZv1cq/view?usp=sharing",
  linkedin: "https://www.linkedin.com/in/pranay-raj-rathod-pathlavath-ab2271289//",
  twitter: "https://x.com/PranayRaj374460",
  insta: "https://www.instagram.com/pranay_raj_rathod/"
};

export const skills = [
  {
    title: "Frontend",
    skills: [
      {
        name: "React Js",
        image:
          "data:image/svg+xml;base64,",
      },
      {
        name: "Next Js",
        image:
          "data:image/png;base64,",
      },
      {
        name: "HTML",
        image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
      },
      {
        name: "CSS",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
      },
      {
        name: "JavaScript",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
      },
      {
        name: "Bootstrap",
        image:
          "https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png",
      },
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "Node Js",
        image: "https://nodejs.org/static/images/logo.svg",
      },
      // {
      //   name: "MongoDB",
      //   image: "https://nodejs.org/static/images/logo.svg",
      // },
      {
        name: "Flask",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTmD38KsMgEwahtWc_Nfs5ZVktP9dBc36MUZA&s",
      },
       {
        name: "Express Js",
        image:
          "data:image/png;base64,",
      },
      // {
      //   name: "Graph Ql",
      //   image: "https://www.vectorlogo.zone/logos/graphql/graphql-icon.svg",
      // },
      // {
      //   name: "Python",
      //   image:
      //     "https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg",
      // },
      // {
      //   name: "MySQL",
      //   image:
      //     "https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg",
      // },
      // {
      //   name: "Postgresql",
      //   image: "https://www.postgresql.org/media/img/about/press/elephant.png",
      // },
      {
        name: "MongoDB",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg",
      },
      // {
      //   name: "Firebase",
      //   image: "https://www.vectorlogo.zone/logos/firebase/firebase-icon.svg",
      // },
    ],
  },
  {
    title: "Machine Learning",
    skills: [
      {
        name: "Pandas",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTCpCB6Du8H6Lrm5WIbDcdW59uqoSiL-eeTlw&s",
      },
      {
        name: "Numpy",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQRC2C9EVtvZjW_wQ3f9bEP2Fgla230C3kVYQ&s",
      },
      {
        name: "OpenCV",
        image: "OpenCV.png",
      },
      {
        name: "Scikit Learn",
        image: "https://upload.wikimedia.org/wikipedia/commons/0/05/Scikit_learn_logo_small.svg",
      },
      {
        name: "Google Colab",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTvyuHWMd6UOi4d_oVuHTBZsGvS7kG6TFK2yQ&s",
      },
      {
        name: "Jypyter NoteBook",
        image: "https://seeklogo.com/images/J/jupyter-logo-A91705F539-seeklogo.com.png",
      },
    ]
  },
  {
    title: "Programming Languages",
    skills: [
      {
        name: "Python",
        image: "https://www.sketchappsources.com/resources/source-image/python-logo.png",
      },
      {
        name: "Java",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRkBkpAn_WwWrYS08ErIEfXMWU_HAlE8LBUPA&s",
      },
      {
        name: "C",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQfKZdbmTKy7h3cd-gxj7dKBQpTUJkt5tpmtQ&s",
      },
      {
        name: "C++",
        image: "https://upload.wikimedia.org/wikipedia/commons/thumb/1/18/ISO_C%2B%2B_Logo.svg/1822px-ISO_C%2B%2B_Logo.svg.png",
      }
    ]
  },
  {
    title: "Others",
    skills: [
      {
        name: "Git",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQGp2RKrzqJF2Cu3dxnk2RGK_F7SSloJG-Wyw&s"  
      },
      {
        name: "GitHub",
        image:
          "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
      },
      {
        name: "Docker",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original-wordmark.svg",
      },
      // {
      //   name: "Netlify",
      //   image:
      //     "https://seeklogo.com/images/N/netlify-logo-BD8F8A77E2-seeklogo.com.png",
      // },
      {
        name: "VS Code",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
      },
    ],
  },
];

export const experiences = [
  {
    id: 0,
    img: "",
    role: "Hackathon Finalist",
    company: "Hack4SDG IIT Hyd",
    date: "26 Oct 2024 - 27 Oct 2024",
    desc: "Worked on a challenging real world problem innovating and developing a solution corresponding to the SDG 12 'Sustainable Cities and Commuunities'! Had a remarkable experience diving into a 24 hour competetive environment",
    skills: [
      "ReactJS",
      "FireBase",
      "NodeJs",
      "Flutter",
      "HTML",
      "CSS",
      "JavaScript",
    ],
    doc: "hack4sdg.png",
  },
  {
    id: 1,
    img: "https://bcopbapatla.in/wp-content/uploads/61cf-fMKNzL.jpg",
    role: "NSS Volunteer",
    company: "National Service Scheme (NSS)",
    date: "Since March 2025",
    desc: "Participating in the NSS Camps was an unforgettable journey. As a member of the PR team, I actively promoted key initiatives such as blood donation, rice donation, eco-friendly Clay Ganesh idols, and tree plantation drives—encouraging widespread community involvement. In my role as a volunteer, I interacted with diverse groups of people, helped coordinate activities, and ensured the events ran smoothly. It was a deeply rewarding experience that allowed me to contribute meaningfully while creating a positive impact on society. ",
    skills: [
      "Communication",
      "Collaboration",
      "Event Management",
      "Leadership",
      "Public Relations",
    ],
  },
  // {
  //   id: 2,
  //   img: "https://firebasestorage.googleapis.com/v0/b/flexi-coding.appspot.com/o/girlScript.jpeg?alt=media&token=e656a621-cf3c-4230-bf0f-e74b4cec6035",
  //   role: "Open Source Contributor ",
  //   company: "GirlScript Summer of Code",
  //   date: "May 2023 - Present",
  //   desc: "Contributing to different open-source projects and learn from industry experts",
  //   doc: "https://media.licdn.com/dms/image/D4D22AQGzSpgFaR4hsQ/feedshare-shrink_800/0/1684079632872?e=1687996800&v=beta&t=i0xWn_k32Oj9SxKfc6rARSbOghXD9Ge6WyEb2p2vvrc",
  // },
  // {
  //   id: 3,
  //   img: "https://firebasestorage.googleapis.com/v0/b/flexi-coding.appspot.com/o/gdsc.jpeg?alt=media&token=c162329c-efaa-4be8-a173-8d3f4c48ea70",
  //   role: "Android Developer",
  //   company: "DSC KIIT",
  //   date: "Nov2021 - Present",
  //   desc: "As an Android developer at the Google Developers Student Club (GDCS), I have had the opportunity to work on exciting projects and collaborate with talented developers who share my passion for technology. Through my involvement with GDCS, I have also had the opportunity to host and participate in numerous events, including hackathons, study jams, and workshops.",
  //   skills: [
  //     "Leadership",
  //     "Mobile Application Development",
  //     "Kotlin",
  //     "XML",
  //     "Figma",
  //   ],
  // },
  // {
  //   id: 4,
  //   img: "https://firebasestorage.googleapis.com/v0/b/flexi-coding.appspot.com/o/Rudraksha.jpeg?alt=media&token=8f83f41e-d0a1-486f-9c7f-dd4cd1d09e8d",
  //   role: "Android Developer Intern",
  //   company: "Rudraksha Welffare Foundation",
  //   date: "June 2021 - Oct 2021",
  //   desc: "As an Android Developer Intern at Rudraksha Welfare Foundation from June 2021 to October 2021, I gained valuable hands-on experience in application development. During my tenure, I contributed to the development of the application's back-end cloud functions using Firebase and implemented a front-end with the MVVM structure, which was designed using Figma. I also added a one-to-one video call feature, conceptualized new features, and optimized app performance. This internship helped me enhance my skills in Android development and gave me an opportunity to work in a collaborative team environment.",
  //   skills: [
  //     "Android",
  //     "Java",
  //     "Kotlin",
  //     "XML",
  //     "Node Js",
  //     "Cloud Firestore",
  //     "Firebase",
  //     "Figma",
  //   ],
  //   doc: "https://firebasestorage.googleapis.com/v0/b/buckoid-917cf.appspot.com/o/WhatsApp%20Image%202023-05-05%20at%2012.07.39%20AM.jpeg?alt=media&token=9f0e1648-568b-422d-bd0b-1f125f722245",
  // },
];

export const education = [
  {
    id: 0,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR8wia9hMu8w5XcpDlxW6il9NWqCXBVuDp3JiKOF6y6JeakmXL7817SAPntD5K87zUkXt4&usqp=CAU",
    school: "Vallurupalli Nageswara Rao Vignana Jyothi Institute of Engineering andd Technology, Hyderabad",
    date: "August 2023 - May 2027 (expected)",
    grade: "9.33 CGPA",
    desc: "I'm currently pursuing my undergraduate degree in Computer Science and Engineering with a specialization in Artificial Intelligence and Machine Learning at VNR VJIET.",
    degree: "Computer Science Engineering (Artificial Intelligence and Machine Learning)",
  },
  {
    id: 1,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSnMBhlNCGEsdjzYm67iR9Qzk1waZshp2Zj-Q&s",
    school: "Rajiv Gandhi University of Knowledge and Technologies, Basar, Nirmal",
    date: "September 2021 - May 2023",
    grade: "9.17 CGPA",
    desc: "I completed my Pre-University course (equivalent to telangana state intermediate) at RGUKT Basar (IIIT Basar) ",
    degree: "Pre-University Course",
  },
  {
    id: 2,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT3wh38G5bWWkrh1ttRbSwe8I7RACLKDwSgzA&s",
    school: "ST. Anthony's hign school, Vidyanagar, Sangareddy",
    date: "Jun 2011 - May 2021",
    grade: "10 CGPA",
    desc: "Completed primary and secondary education (Grades 1–10) in Sangareddy, laying a strong academic foundation.",
    degree: "SSC",
  },  
];

export const projects = [
  {
    id: 1,
    title: "Crop Recommendation",
    date: "March 2025",
    description:
      "Trained a Random Forest model to predict the crop based on the parameters like humidity, Temperature, Nitrogen, Potassium and few factors in the soil and at the place. The model reached an accuracy of 99.3%",
    image:
      "CropPrediction.png",
    tags: ["Python", "Random Forest", "Data Enigneering"],
    category: "machine learning",
    github: "https://github.com/Pranayrajrathod/CropPrediction",
    webapp: "https://github.com/Pranayrajrathod/CropPrediction",
    // member: [
    //   {
    //     name: "Rishav Chanda",
    //     img: "https://avatars.githubusercontent.com/u/64485885?v=4",
    //     linkedin: "https://www.linkedin.com/in/rishav-chanda-b89a791b3/",
    //     github: "https://github.com/rishavchanda/",
    //   },
    //   {
    //     name: "Upasana Chaudhuri",
    //     img: "https://avatars.githubusercontent.com/u/100614635?v=4",
    //     linkedin: "https://www.linkedin.com/in/upasana-chaudhuri-2a2bb5231/",
    //     github: "https://github.com/upasana0710",
    //   },
    // ],
  },
  {
    id: 2,
    title: "Waste Reporting System",
    date: "Oct 2024 - May 2025",
    description:
      "A web application where a user can report and earn out of the waste",
    image:
      "waste.png",
    tags: ["Mongo DB","React JS","Node JS","BootStrap","HTML","CSS"],
    category: "web app",
    github: "https://github.com/Pranayrajrathod/mern-project-wasteReportingSystem",
    webapp: "https://github.com/Pranayrajrathod/mern-project-wasteReportingSystem",
  },
  {
    id: 3,
    title: "Face and Eye Detection",
    date: "Jun 2025",
    description:
      "Using OpenCV to detect faces and eyes from web cam and images uploaded. I have used haar cascade classifier to detect faces and eyes",
    image:
      "LiveFaceAnd EyeDetection.png",
    tags: ["OpenCV", "HaarCascade", "Machine Learning","Computer Vision"],
    category: "machine learning",
    github: "https://github.com/Pranayrajrathod/FaceAndEyeDetection",
    webapp: "https://github.com/Pranayrajrathod/FaceAndEyeDetection",
  },
  {
    id: 4,
    title: "Fake News Detections",
    date: "Feb 2025",
    description:
      "Trained a logistic regression model to predit a news to be fake or real, by processing a dataset from kaggle which reachead a high accuracy of 93%",
    image:
      "FakeNews.png", 
    tags: ["Python","Data Engineering","Logistic Regression"],
    category: "machine learning",
    github: "https://github.com/Pranayrajrathod/FakeNewsDetection/tree/main",
    webapp: "https://github.com/Pranayrajrathod/FakeNewsDetection/tree/main",
  },
  {
    id: 5,
    title: "Civic Link",
    date: "Sept 2025",
    description:
      "I developed CivicLink, a community-driven platform for reporting civic issues through audio, text, or image inputs. Using Gemini for AI-powered analysis, the system prioritizes reports based on severity and dynamically assigns tasks to workers. It features a chatbot for user support and an admin dashboard for centralized oversight. This project aims to enhance civic engagement and streamline issue resolution in communities.",
    image:
      "CivicLink.png",
    tags: ["React","Flask","Gemini","MongoDB","Bootstrap"],
    category: "web app",
    github: "https://github.com/Pranayrajrathod/CivicLink",
    webapp: "https://github.com/Pranayrajrathod/civicLink",
    member: [
      {
        name: "Pranay Raj Rathod",
        //import image from images in src/images/HeroImage.png
        img: "https://media.licdn.com/dms/image/v2/D5603AQE-POPb6j0IGQ/profile-displayphoto-shrink_400_400/profile-displayphoto-shrink_400_400/0/1701190303336?e=1760572800&v=beta&t=5Zeo8F6ePPkRa09CPLRvT_sCiY50oUFH6OtBlnE6xOY",
        linkedin: "https://www.linkedin.com/in/pranay-raj-rathod-743a3b231/",
        github: "https://github.com/Pranayrajrathod/",
      },
      {
        name: "Sai Praneeth K",
        img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQmRLRMXynnc7D6-xfdpeaoEUeon2FaU0XtPg&s",
        linkedin: "https://www.linkedin.com/in/sai-praneeth-kummari-1038b729b/",
        github: "https://github.com/saipraneeth-12",
      },
      {
        name: "Janardhan R",
        img: "https://media.licdn.com/dms/image/v2/D4E03AQHiSVpD1juZ2g/profile-displayphoto-shrink_400_400/B4EZR9QumGGcAg-/0/1737268325668?e=1760572800&v=beta&t=g8Lft1g6AwqN_viv8Xy65PYjcMAjVu1BzGzx4_4tgus",
        linkedin: "https://www.linkedin.com/in/janardhan93/",
        github: "https://github.com/Janardhan-225",
      },
    ],
  },
];

export const codingProfiles = [
  {
    platform: "LeetCode",
    username: "Pranayrajrathod",
    description: "solved over 220+ questions and participated in many contests. Click to view profile",
    link: "https://leetcode.com/u/pranay_raj_rathod/",
    tags: ["DSA", "Contests", "Algorithms", "LeetCode"],
  },
  {
    platform: "CodeChef",
    username: "pranay_01",
    description: "Regularly participating in contests. Click to view profile",
    link: "https://www.codechef.com/users/pranay_raj09",
    tags: ["Competitive Programming", "Problem Solving", "CodeChef","Contests"],
  },
  {
    platform: "Codeforces",
    username: "pranayraj",
    description: "Engaged in many contests. Click to view profile",
    link: "https://codeforces.com/profile/pranayraj904",
    tags: ["Competitive Programming", "Contests", "Codeforces"],
  },
];

export const TimeLineData = [
  { year: 2021, text: "Joined IIIT Basar, pursued Pre University Course" },
  { year: 2023, text: "Joined VNR VJIET to pursue BTech in CSE-AI&ML" },
  { year: 2024, text: "Started Building projects" },
  { year: 2025, text: "Created my portfolio" }, 
];
